...
public:
    using PresentCallback = std::function<HRESULT(const RECT*, const RECT*, HWND, const RGNDATA*)>;
...
private:
    HRESULT InvokePresent(const RECT *pSourceRect, const RECT *pDestRect, HWND hDestWindowOverride,
                          const RGNDATA *pDirtyRegion);

    IDirect3DDevice9* _dx;
    PresentCallback _presentCallback;
};
